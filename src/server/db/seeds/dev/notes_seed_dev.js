...
const colors = ['$green-grass', '$aqua-light', '$agua-dark', '$lemon-yellow', '$orange', '$deep-rose',
    '$sky-blue', '$classic-red', '$lavender', '$dusty-sand', '$sunset', '$cake-pink', null, null, null, null, null];
    ...
    return knex('notes').del()
        .then(function () {
            ...
            for (let j = 0; j < 100; j++) {
                ...
                let orientRand = null;
                if (colorRand != null) {
                    orientRand = orientation[Math.floor(Math.random() * orientation.length)];
                }

                data.push({
                    title: headerRand.join(' '),
                    body: bodyRand.join(' '),
                    photo: colorRand,
                    orientation: orientRand
                });
